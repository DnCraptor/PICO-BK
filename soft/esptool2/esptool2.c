...
//   chksum - pointer to existing checksum to add this data to (zero if not needed)
// Produces error message on failure (so caller doesn't need to).
bool WriteElfSegment(MyElf_File *elf, FILE *outfile, Elf32_Phdr *seg, bool headed,
	bool zeroaddr, int padto, unsigned char *chksum) {
	...
	// create image section header
	sechead.addr = (zeroaddr ? 0 : seg->p_paddr);
	sechead.size = seg->p_filesz;
	...
	debug("Adding segment at 0x%08lX, size: %d (+%d bytes(s) padding).\r\n",
		seg->p_paddr, seg->p_filesz, pad);

	// get elf section binary data
	bindata = GetElfSegmentData(elf, seg);
	if (!bindata) {
		goto end_function;
	}
	...
	// write section (and pad if required)
	if((headed && fwrite(&sechead, 1, sizeof(sechead), outfile) != sizeof(sechead))
		|| fwrite(bindata, 1, seg->p_filesz, outfile) != seg->p_filesz
		|| (pad > 0 && fwrite(PADDING, 1, pad, outfile) != pad)) {
		error("Error: Failed to write segment at 0x%08lX to image file.\r\n", seg->p_paddr);
		goto end_function;
	}
	
	// include section data in the checksum
	if(chksum) {
		for(i = 0; i < (int)seg->p_filesz; i++) {
			*chksum ^= bindata[i];
		}
	}
...
bool CreateBinFile(char *elffile, char *imagefile, int bootver, unsigned char mode,
	unsigned char clock, unsigned char size, bool iromchksum) {

	bool ret = false;
	int pad, len, iSeg, numseg;
...
{
	int Count;
	for (Count = 1; Count < elf->header.e_shnum; Count++)
	{
		debug
		(
			"Sec%2u: offset=0x%08"PRIX32" address=0x%08"PRIX32" Size=0x%08"PRIX32" Name=%s\n",
			Count,
			elf->sections [Count - 1].offset,
			elf->sections [Count - 1].address,
			elf->sections [Count - 1].size,
			elf->sections [Count - 1].name
		);
	}

	debug ("e_phnum = %d\n", (int) elf->header.e_phnum);
	fflush (stdout);
	for (Count = 0; Count < elf->header.e_phnum; Count++)
	{
		debug
		(
			"Seg%2u: offset=0x%08"PRIX32" vaddress=0x%08"PRIX32" paddress=0x%08"PRIX32" FSize=0x%08"PRIX32" MSize=0x%08"PRIX32" Flags=0x%08"PRIX32" Align=0x%08"PRIX32"\n",
			Count,
			elf->segments [Count].p_offset,
			elf->segments [Count].p_vaddr,
			elf->segments [Count].p_paddr,
			elf->segments [Count].p_filesz,
			elf->segments [Count].p_memsz,
			elf->segments [Count].p_flags,
			elf->segments [Count].p_align
		);
	}
}
	...
	if (bootver == 2) {
		...
		for (iSeg = 0, numseg = 0; iSeg < elf->header.e_phnum; iSeg++)
		{
			if (elf->segments [iSeg].p_filesz && (elf->segments [iSeg].p_paddr >= 0x40200000))
			{
				if(numseg == 0) {
					if(!WriteElfSegment(elf, outfile, &elf->segments [iSeg], true, true, IMAGE_PADDING, (iromchksum ? &chksum : 0))) {
						goto end_function;
					}
				}
				numseg++;
		...
		if (numseg == 0) {
		    error("Error: IROM segment not found in elf file.\r\n");
			goto end_function;
		}
		else if (numseg > 1) {
		    error("Error: More then one IROM segments found in elf file.\r\n");
			goto end_function;
		}
	}
	...
	for (iSeg = 0, numseg = 0; iSeg < elf->header.e_phnum; iSeg++)
	{
		if (elf->segments [iSeg].p_filesz && (elf->segments [iSeg].p_paddr < 0x40200000)) numseg++;
	}

	// standard header
    imghead.magic = BIN_MAGIC_FLASH;
    imghead.count = numseg;
	// write header
	if(fwrite(&imghead, 1, sizeof(imghead), outfile) != sizeof(imghead)) {
		error("Error: Failed to write header to image file.\r\n");
		goto end_function;
	...
	// add segments
	for (iSeg = 0; iSeg < elf->header.e_phnum; iSeg++)
	{
		if (elf->segments [iSeg].p_filesz && (elf->segments [iSeg].p_paddr < 0x40200000)) {
			if(!WriteElfSegment(elf, outfile, &elf->segments [iSeg], true, false, SECTION_PADDING, &chksum)) {
				goto end_function;
			}
		}
	}
	...
	if(bootver == 1) {
		...
		for (iSeg = 0, numseg = 0; iSeg < elf->header.e_phnum; iSeg++)
		{
			...
			{
				if(numseg == 0) {
					if(!WriteElfSegment(elf, outfile, &elf->segments [iSeg], false, false, 0, 0)) {
						goto end_function;
					}
				}
				numseg++;
...
int main(int argc, char *argv[]) {
	...
	if (argc < 2) {
		...
		print("esptool2 -bin [options] <input_file> <output_file>\r\n");
		...
		print("\r\n");
		print("  -bin\r\n");
		print("       Create binary program image, for standalone and bootloaded apps.\r\n");
		print("       Includes sdk library for bootloaded apps.\r\n");
		print("       e.g. esptool2 -bin esp8266_iot.out out.bin\r\n");
		print("       Options:\r\n");
	...
	if (((libfile | binfile) && i + 2 > argc) || ((headerfile) && i + 3 > argc)) {
		error("Error: Not enough arguments supplied.\r\n");
		return -1;
	} else if ((libfile | binfile) && i + 2 < argc) {
	...
	// do it
	if (binfile) {
		if (!CreateBinFile(infile, outfile, bootver, mode, clock, size, iromchksum)) {
			remove(outfile);
			return -1;
		}
	} else if (headerfile) {
