...
unsigned char* GetElfSegmentData(MyElf_File *elf, Elf32_Phdr *seg) {

    unsigned char *data = 0;
    
	if (seg->p_filesz && seg->p_offset) {

		data = (unsigned char*)malloc(seg->p_filesz);
		if(!data) {
	        error("Error: Out of memory!\r\n");
			return 0;
		}
            
		if(fseek(elf->fd, seg->p_offset, SEEK_SET) ||
		   fread(data, 1, seg->p_filesz, elf->fd) != seg->p_filesz) {
			error("Error: Can't read segment at 0x%08lX data from elf file.\r\n", seg->p_paddr);
			free(data);
			return 0;
	    }

	} else {
		error("Error: Segment at 0x%08lX has no data to read.\r\n", seg->p_paddr);
	}
...
MyElf_File* LoadElf(char *infile) {
	...
	elf->segments = (Elf32_Phdr*)malloc(sizeof(Elf32_Phdr) * elf->header.e_phnum);
	if(!elf->segments) {
		error("Error: Out of memory!\r\n");
        goto error_exit;
	}
	for(i = 0; i < elf->header.e_phnum; i++) {
        if(fseek(elf->fd, elf->header.e_phoff + (elf->header.e_phentsize * i), SEEK_SET)
		   || fread(&elf->segments [i], 1, sizeof(Elf32_Phdr), elf->fd) != sizeof(Elf32_Phdr)) {
            error("Error: Can't read segment %d from elf file.\r\n", i);
	        goto error_exit;
		}
		debug("Read segment %d at 0x%08lX size=0x%08lX.\r\n", i+1, elf->segments [i].p_paddr, elf->segments [i].p_filesz);
	}
...
error_exit:
	if (elf) {
		...
		if (elf->segments) free(elf->segments);
		free(elf);
	}
	return 0;
